...
void StartTask02(void *argument)
{
   ...
   *   位置目标/实测 mRad 输出轴(09-10加),
   *   上电 SPI 21-bit 绝对角锁存(09-17加,每帧平线,跨上电重复性直读) —— 共 11 列
   ...
   *   \r 与 \n 均作行终止且不入缓冲——09-17 加）：
   *   ⚠ 严格校验（09-17 幽灵加固）：合法行=单字母[空格[−]数字[.数字]]到行尾，
   *      非法整行拒收打 [x] REJECT: 原文，不进任何命令分支——HL-340 适配器在
   *      端口开合瞬间泄漏的陈旧字节（plot0/p 100）拼进首条命令时在此拦下；
   *      实机三环全开状态验证过拦截、14 命令零误拦
   ...
   *           超温 80°C 另有 [ntc] 报警行）
   *   m / l / g  重力前馈（09-14）：负载质量 kg / 力臂 m / 开关（g 1 开 g 0 关，
   *           上电默认关；开前确认上电时摆自由悬底，否则相位错）
   * 过温跳闸（09-05 晚）：NTC ≥70°C 或开路（< -30°C）→ iq_ref 强制 0 + 拒绝 q/s/z/v；
   * 过温降回 60°C 自动解除（解除后不出力，须重新下发命令）；开路锁存断电才复位
   *
   * ECHO_TEST=1（foc.h，取证模式）：不起环/DRV 禁用（main.c 同开关门控），命令
   *   处理换成纯回声台——ECHO=形态合法 / GHOST=非法（幽灵）分类回显，不执行；
   *   正常烧录保持 ECHO_TEST=0 */
  ...
  {
#if ECHO_TEST
    /* ==================== 纯回声台（取证固件，foc.h ECHO_TEST=1）====================
     * 收到的行原样回显并分类：形态合法打 ECHO、非法打 GHOST（幽灵字节拼进的行），
     * 不可打印字节以 <hh> 十六进制显形——行内 NUL 会截断普通字符串显示丢证据，
     * 故显示循环以 uart_cmd_idx 为界、不看 '\0'。分类判据与正式版严格校验同源。
     * 只回声不执行：不起环不碰 DRV（main.c 同开关已门控），CSV 照常打印 */
    if (uart_cmd_ready)
    {
      uart_cmd_buf[uart_cmd_idx] = '\0';
      uint8_t valid = 0;
      {
        const char *p = (const char *)uart_cmd_buf;
        if (p[0] >= 'a' && p[0] <= 'z')
        {
          if (p[1] == '\0')
            valid = 1;
      ...
      char ebuf[140];
      int elen = snprintf(ebuf, sizeof(ebuf), "%s:%u:[", valid ? "ECHO" : "GHOST",
                          (unsigned int)uart_cmd_idx);
      for (uint8_t i = 0; i < uart_cmd_idx && elen < (int)sizeof(ebuf) - 8; i++)
      {
        char ch = uart_cmd_buf[i];
        if (ch >= 0x20 && ch < 0x7F)
          ebuf[elen++] = ch;
        else
          elen += snprintf(ebuf + elen, 5, "<%02X>", (unsigned char)ch);
      }
      if (elen < (int)sizeof(ebuf) - 3)
      {
        ebuf[elen++] = ']';
        ebuf[elen++] = '\r';
        ebuf[elen++] = '\n';
      ...
      if (elen > 0)
        HAL_UART_Transmit(&huart1, (uint8_t *)ebuf, (uint16_t)elen, 20);
      uart_cmd_idx = 0;
      uart_cmd_ready = 0;
    }
#else
    if (uart_cmd_ready)
    {
      ...
      /* 命令严格校验（09-17 幽灵加固）：合法行=单字母[空格[−]数字[.数字]]到行尾，
       * 其余（幽灵拼接前缀/数字后残留/空行）整行拒收——不进任何命令分支 */
      uint8_t cmd_ok = 0;
      {
        const char *p = (const char *)uart_cmd_buf;
        if (p[0] >= 'a' && p[0] <= 'z')   /* 命令字恰好 1 个：首字母后只能是空格或行尾 */
        {
          if (p[1] == '\0')
            cmd_ok = 1;                    /* 裸字母命令：z/c/w/t/d */
          else if (p[1] == ' ')
          {
            p += 2;
            if (*p == '-') p++;            /* 可选负号 */
            if (*p >= '0' && *p <= '9')    /* 至少一位整数，小数可选，其后必须行尾 */
            {
              cmd_ok = 1;
              while (*p >= '0' && *p <= '9') p++;
              if (*p == '.')
      ...
      /* 简易浮点解析（照抄 G431）：空格后的 [+|-]数字[.数字]（校验已过必无残留） */
      float val = 0.0f;
      {
     ...
     * 重新下发命令才出力；短路读极热→≥70°C 跳闸路。开路/短路两失效方向均 fail-safe。
