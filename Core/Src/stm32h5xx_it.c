...
/* Private variables ---------------------------------------------------------*/
/* USER CODE BEGIN PV */
extern volatile uint32_t foc_dt_last;   /* main.c：GPDMA0 ISR 耗时统计 */
...
void GPDMA1_Channel0_IRQHandler(void)
{
  ...
  uint32_t foc_t0 = DWT->CYCCNT;   /* ISR 耗时测量入口：覆盖 HAL 分发+FOC+SVPWM+重挂全链 */
  /* USER CODE END GPDMA1_Channel0_IRQn 0 */
  HAL_DMA_IRQHandler(&handle_GPDMA1_Channel0);
  /* USER CODE BEGIN GPDMA1_Channel0_IRQn 1 */
  uint32_t foc_dt = DWT->CYCCNT - foc_t0;   /* 差值对 32 位回绕免疫 */
  foc_dt_last = foc_dt;
  foc_dt_sum += foc_dt;
  foc_dt_n++;
  if (foc_dt > foc_dt_max) foc_dt_max = foc_dt;
