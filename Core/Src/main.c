...
int main(void)
{
  ...
  *(volatile uint32_t *)0xE0001FB0UL = 0xC5ACCE55UL;
   ...
   * 电机静止时无电流 */
#if ECHO_TEST
  /* echo 模式：DRV 保持禁用（gpio.c 初低），功率级硬关——电机坠落检修期双保险 */
#else
  ...
  uint16_t boot_cnt = 0;   /* 位置环零点=上电播种位置：必须在对齐之前记（09-09 设计） */
  {
    ...
    shadow_boot_ang = ang;   /* 21-bit 原始绝对角锁存：CSV 第 11 列持续平线输出（电机座 21 点实验） */
...
#if ECHO_TEST
  /* echo 模式：跳过对齐与起环（详见 foc.h ECHO_TEST 注释）；
   * CSV 打印照常——链路对照需在与事故相同的满载数据流下进行 */
  {
    static const char msg[] = "[ECHO] firmware: loops off, DRV off, commands echoed\r\n";
    HAL_UART_Transmit(&huart1, (uint8_t *)msg, sizeof(msg) - 1, 20);
  }
#else
  /* 零点对齐：v2 闭环双侧逼近（09-05 版）。
   * ⚠ 09-17 曾试电零点表直读方案（对齐退役+常数 55188+21 锚点插值）——首烧即
   * 电机剧烈摆动，当晚回退本对齐版；表方案数据已留档 logs/0917_2305，待分析定因
   * （嫌疑：常数提取符号域错 / 表插值符号反）。ZeroAlign 兼职开 PWM 通道——
   * 电流环只写 CCR 从不开通道，勿再单独退役对齐 */
  FOC_ZeroAlign(&g_foc);
