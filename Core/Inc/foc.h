...
/* ====== echo 对照实验开关（2026-09-17 建，实验完改回 0） ======
 * 1 = 固件变纯回声台：不起环、不对齐、DRV 保持禁用（gpio.c 初低，功率级硬关）；
 *     收到的每条命令原样回传（不可打印字节以 <hh> 十六进制显形），不执行任何命令。
 * 动机：p 100 幽灵命令致电机坠落事故的取证——判定篡改发生在 VOFA+/hub/TCP
 * 链路，还是松动 USB 连接器的信号劣化。CSV 打印照常（满载数据流下测试）。 */
#define ECHO_TEST       0

/* ===== 电零点 21 锚点校正（2026-09-15 标定/09-16 提取/09-17 入固件） =====
 * 方案：上电对齐退役，电角度 = cnt×PP×ENC_DIR + ELEC_OFFSET_DIRECT + e_tab 插值
 * （表在 foc.c，域=cnt mod 65536、锚点 k×65536/21，corr=−e×65536/360）。
 * ELEC_OFFSET_DIRECT = 标定会话 elec_offset16（重力负载3.docx θ/pact 列 506 帧圆均值，
 * 提取不确定度 ~±0.5°）。预期收益：id_DC@880 ≤±1.5A（vs 对齐抽奖最坏 −9.1A），
 * 且上电无 5A 对齐拽转子。z 命令保留=诊断（运行时覆盖 offset，重启恢复常数）。
 * 残差验证规程：悬底上电 → s 100 正 3 输出圈 → s −100 反 2.5 圈（只发 s 命令）→
 * id 残差应≈常数；非常数=表失效（磁铁挪动），常数非零=修正本值一次即闭环。
 * 表与本常数随 ⌀6 磁铁+编码器总成，⌀10 到货按规程重标（见根目录 电零点标定表.md） */
#define ELEC_OFFSET_DIRECT  55188
...
extern volatile uint32_t shadow_boot_ang; /* 上电 SPI 21-bit 绝对角锁存（CSV 第 11 列，电机座标定用） */
